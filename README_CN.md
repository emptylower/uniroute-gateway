...
本分支包含 ShipAny 控制平面对接：账号、权限、支付、积分和 API Key 生命周期归 ShipAny，Sub2API 只承担模型流量、上游账号、路由、Redis 状态和用量记录。

服务器部署、密钥映射、Cloudflare 联调、备份与回滚步骤见 [UniRoute Gateway + ShipAny 部署手册](deploy/SHIPANY_GATEWAY_DEPLOYMENT.md)。当前权威钱包仍处于影子阶段，文档中的联调模式不得直接用于正式收费切流。
