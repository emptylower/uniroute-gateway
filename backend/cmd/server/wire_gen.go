...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	channelPreferenceRepository := repository.NewChannelPreferenceRepository(db)
	...
	channelPreferenceService := service.NewChannelPreferenceService(channelPreferenceRepository, apiKeyService, channelService, apiKeyAuthCacheInvalidator)
	apiKeyHandler := handler.ProvideAPIKeyHandler(apiKeyService, channelPreferenceService)
	...
	modelObservationRepository := repository.NewModelObservationRepository(db)
	upstreamConnectionRepository := repository.NewUpstreamConnectionRepository(client, db)
	accountTestService := service.ProvideAccountTestService(accountRepository, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, openAIGatewayService, modelObservationRepository, upstreamConnectionRepository)
	...
	modelRegistryRepository := repository.NewModelRegistryRepository(db)
	modelRegistryService := service.NewModelRegistryService(modelRegistryRepository)
	modelRegistryHandler := admin.NewModelRegistryHandler(modelRegistryService)
	shadowDecisionRepository := repository.NewShadowDecisionRepository(db)
	modelClassifier := service.NewModelClassifier()
	publicationEvaluator := service.NewPublicationEvaluator()
	accountEndpointProbeRepository := repository.NewAccountEndpointProbeRepository(client, db)
	endpointProbeChecker := service.ProvideEndpointProbeChecker(accountEndpointProbeRepository)
	channelPriceChecker := service.ProvideChannelPriceChecker(db)
	billingMappingChecker := service.ProvideBillingMappingChecker(db)
	resourceChecker := service.ProvideResourceChecker(db)
	...
	modelCatalogSnapshotStore := repository.NewModelCatalogSnapshotRepository(db)
	modelCatalogEvidenceReader := repository.NewModelCatalogEvidenceRepository(db)
	modelCatalogCandidateService := service.ProvideModelCatalogCandidateService(modelCatalogSnapshotStore, modelCatalogEvidenceReader, modelRegistryRepository, auditLogRepository, configConfig)
	modelCatalogCandidateHandler := admin.NewModelCatalogCandidateHandler(modelCatalogCandidateService)
	upstreamConnectionService := service.ProvideUpstreamConnectionService(upstreamConnectionRepository, secretEncryptor)
	aggregatorDesignationService := service.ProvideAggregatorDesignationService(upstreamConnectionRepository)
	aggregatorReuseRepository := repository.NewAggregatorReuseRepository(client, db)
	aggregatorConnectionReuseService := service.ProvideAggregatorConnectionReuseService(upstreamConnectionRepository, accountRepository, aggregatorReuseRepository, accountEndpointProbeService, secretEncryptor)
	modelGovernanceReadRepository := repository.NewModelGovernanceReadRepository(db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, modelGovernanceInventoryHandler, modelRegistryHandler, modelGovernanceService, upstreamBillingProbeService, ollamaCloudUsageService, modelQuarantineService, modelAuthorizationActivationService, modelCatalogCandidateHandler, accountEndpointProbeService, upstreamConnectionService, aggregatorDesignationService, aggregatorConnectionReuseService, accountRepository, upstreamConnectionRepository, modelGovernanceReadRepository)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, channelRoutingSelector, coordinator)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator, channelRoutingSelector)
	...
	platformIdentityRepository := repository.NewPlatformIdentityRepository(client, configConfig)
	platformIdentityService := service.NewPlatformIdentityService(platformIdentityRepository)
	platformIdentityHandler := handler.NewPlatformIdentityHandler(platformIdentityService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, userService, platformIdentityService, compositeRouteResolver, redisClient)
	...
	application := &Application{
		...
		ModelCatalogCandidate: modelCatalogCandidateService,
		Cleanup:               v,
	}
