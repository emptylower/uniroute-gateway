...
func (h *UserHandler) Update(c *gin.Context) {
	...
	if req.BillingCurrency != nil {
		...
		requested, normalizeErr := service.NormalizeBillingCurrency(*req.BillingCurrency)
		if normalizeErr != nil {
			response.BadRequest(c, normalizeErr.Error())
			return
		}
		if requested != service.NormalizeUserBillingCurrency(target.BillingCurrency) &&
			!middleware.EnforceStepUp(c, h.totpService, h.userService, h.settingService) {
			return
		}
	}
