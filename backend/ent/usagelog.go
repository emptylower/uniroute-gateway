...
type UsageLog struct {
	...
	// Concrete request target platform captured for model governance
	GovernanceTargetPlatform *string `json:"governance_target_platform,omitempty"`
	// 渠道 ID
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldGovernanceTargetPlatform:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field governance_target_platform", values[i])
			} else if value.Valid {
				_m.GovernanceTargetPlatform = new(string)
				*_m.GovernanceTargetPlatform = value.String
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.GovernanceTargetPlatform; v != nil {
		builder.WriteString("governance_target_platform=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")
