...
// The AccountEndpointProbeFunc type is an adapter to allow the use of ordinary function as a Querier.
type AccountEndpointProbeFunc func(context.Context, *ent.AccountEndpointProbeQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f AccountEndpointProbeFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.AccountEndpointProbeQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.AccountEndpointProbeQuery", q)
}

// The TraverseAccountEndpointProbe type is an adapter to allow the use of ordinary function as Traverser.
type TraverseAccountEndpointProbe func(context.Context, *ent.AccountEndpointProbeQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseAccountEndpointProbe) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseAccountEndpointProbe) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AccountEndpointProbeQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.AccountEndpointProbeQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.AccountEndpointProbeQuery:
		return &query[*ent.AccountEndpointProbeQuery, predicate.AccountEndpointProbe, accountendpointprobe.OrderOption]{typ: ent.TypeAccountEndpointProbe, tq: q}, nil
