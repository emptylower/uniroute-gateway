...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("rate_multiplier").
			...
			Default(1.0),
		field.Float("rate_multiplier_cny").
			SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"}).
			Optional().
			Nillable().
			Comment("CNY settlement multiplier; NULL falls back to rate_multiplier"),
		field.Float("rate_multiplier_usd").
			SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"}).
			Optional().
			Nillable().
			Comment("USD settlement multiplier; NULL falls back to rate_multiplier"),
